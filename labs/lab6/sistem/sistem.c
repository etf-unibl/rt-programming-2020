...
static ssize_t sysfs_cell_state_show(struct kobject *kobj,
                struct kobj_attribute *attr, char *buf);
static ssize_t sysfs_cell_state_store(struct kobject *kobj,
                struct kobj_attribute *attr,const char *buf, size_t count);
               
static int stat_value[4] ={0,0,0,0} ;

static char cell_state[100];


struct kobj_attribute stat_attr = __ATTR(stat_value,0660, sysfs_show, sysfs_store);
struct kobj_attribute cell_state_attr = __ATTR(cell_state,0660, sysfs_cell_state_show, sysfs_cell_state_store);
...
static ssize_t sysfs_show(struct kobject *kobj,
                struct kobj_attribute *attr, char *buf)
{
        printk(KERN_INFO "Sysfs - Read - stat_value!!!\n");
        printk(KERN_INFO "Zive : %d \n",stat_value[0]);
        printk(KERN_INFO "Rodjene: %d \n",stat_value[1]);
        printk(KERN_INFO "Umrle : %d \n",stat_value[2]); 
        printk(KERN_INFO "Gen: %d \n",stat_value[3]);
        return sprintf(buf, "Zive: %d Rodjene: %d Umrle: %d Generacija: %d\n", stat_value[0],stat_value[1], stat_value[2],stat_value[3]);
}
 
static ssize_t sysfs_store(struct kobject *kobj,
                struct kobj_attribute *attr,const char *buf, size_t count)
{
        printk(KERN_INFO "Sysfs - Write - stat_value!!!\n");
                ...
                struct kobj_attribute *attr,const char *buf, size_t count)
{
    ...
    printk(KERN_INFO"Sysfs - Write - cell_state!!!\n");
    memcpy(cell_state,buf,strlen(buf)+1);
    return count;
                ...
                struct kobj_attribute *attr, char *buf)
{
  ...
  int j; 
  int count=0; 
  printk(KERN_INFO"Sysfs - Read - cell_state!!!\n");
  memcpy(buf,cell_state,strlen(cell_state)+1); 
  //char buff[10];

  for(i=0;i<10;i++)
   { 
      for(j=0;j<10;j++){
       printk("%c",cell_state[(i*10)+j]); 
       count++; 
      }
      printk(" \n");
   }
...
static int __init stat_driver_init(void)
{
        ...
        if(sysfs_create_file(kobj_ref,&stat_attr.attr)){
                printk(KERN_INFO"Cannot create sysfs file (stat_value) ......\n");
                goto r_sysfs_stat;
        }

         //Kreiranje fajla za cell_state u kojem cuvamo trenurno stanje celija
        if(sysfs_create_file(kobj_ref,&cell_state_attr.attr)){
                printk(KERN_INFO"Cannot create sysfs file (cell_state) ......\n");
                goto r_sysfs_cell;
        }
...
r_sysfs_cell:
        ...
        sysfs_remove_file(kernel_kobj, &cell_state_attr.attr);
