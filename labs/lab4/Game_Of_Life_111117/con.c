...
void get_n(char* states,char x, char y){
	...
	char end1=x+1;
	char end2=y+1;
	for(char i=x-1;i<=end1;i++){
		for(char j=y-1;j<=end2;j++){
...
void* thread(void* arg){
	...
	while(1){
		...
		for(char i=0;i<a->wait;i++){
			sem_wait(&(go[a->x][a->y]));
		}
		/*
		 * Takodje kada zavrse evoluciju opet cekaju na semaforu, ako se ustanovi da su sve celije umrle vrijednost promjenljive end
		 * postaje 1 i izlazi se iz petlje.
		 * */
		if(end==1){
			break;
		}
...
//Ova funkcija racuna i vraca broj susjeda na koje mora cekati celija da bi dobila pravo da posne sa izvrsavanjem u novom ciklusu.
...
int main(int argc, char* argv[]){
	 ...
	 * Potrebno je samo paziti da se sem_init ne pozove nad vec inicijalizovanim semaforom pa stoga kod ove funkcije nema
	 * potrebe provjeravati vrijednost.
	 * */
	for(char i=0;i<NUM;i++){
		for(char j=0;j<NUM;j++){
			sem_init(&(go[i][j]),0,0);
	...
	if(pthread_create(&l,NULL,listen,NULL)!=0){
		end=1;
	}
	...
	if(end!=0){
		if(i==NUM){
			//ako je i stigao da se uveca na NUM onda su sve celijske niti su bile inicijalizovane i j je takodje NUM
			//greska se onda desila kod listen niti
			i=NUM-1;
			j=NUM-1;
		}
		else{
			//ako gornji uslov nije ispunjen i!=NUM, a j<=NUM-1 pa umanjujemo j jer se na inicijalizaciji [i][j] celije doslo do greske
			j--;
		}
		for(;i>=0;i--){
			while(j>=0){
